...
if (!sql) {
    const select = cmds.select || 'Id, PostId, Length, MaxCharCount, MaxWord, MaxWordCount, NonAlphaCount, NonAsciiCount'
    const orderBy = cmds.orderBy ? cmds.orderBy : cmds.orderByDesc ? `${cmds.orderByDesc} DESC` : 'Length DESC'
    const where = cmds.where || '1=1'
    const skip = cmds.skip || 0
    const take = cmds.take || 20
    const groupBy = cmds.groupBy ? 'GROUP BY ' + cmds.groupBy : ''
    sql =`SELECT ${select} FROM Stat WHERE ${where} ${groupBy} ORDER BY ${orderBy} LIMIT ${take} OFFSET ${skip}`
}
